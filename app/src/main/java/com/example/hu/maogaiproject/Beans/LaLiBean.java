...
public class LaLiBean implements ISensorInf {
    ...
    //calculate:计算
    //解析串口数据包，并且更新Bean的属性（共33位）
    public void caculate(byte[] bRec) {
        this.bRec = bRec;
        time = System.currentTimeMillis();
        powerVel = MyFunc.twoBytesToInt(bRec, 29);
        assiVel = MyFunc.HexToInt(MyFunc.Byte2Hex(bRec[31]));
    }
