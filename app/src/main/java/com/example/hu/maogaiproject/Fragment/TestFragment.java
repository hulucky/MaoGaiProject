package com.example.hu.maogaiproject.Fragment;
...
import com.example.hu.maogaiproject.Adapter.NvQianYinLiAdapter;
import com.example.hu.maogaiproject.Application.MyApp;
import com.example.hu.maogaiproject.Dialog.SaveDialog;
...
import com.example.hu.maogaiproject.SerialControlUtils.SerialControl;
import com.example.hu.maogaiproject.Utils.MyFunc;
        ...
        if (comBeanLali.recData.length > 0) {
            ...
            if (type == 33) {
                if (buffer.length == 33) {
                    ...
                    float value1 = (float) MyFunc.twoBytesToInt(buffer, 15) / 1000f;
                    if (buffer[14] == 1) {//协议里面，0：正，1：负
                        value1 = -value1;
                    }
                    float value2 = (float) MyFunc.twoBytesToInt(buffer, 18) / 1000f;
                    if (buffer[17] == 1) {
                        value2 = -value2;
                    }
                    float value3 = (float) MyFunc.twoBytesToInt(buffer, 21) / 1000f;
                    if (buffer[20] == 1) {
                        value3 = -value3;
                    }
                    float value4 = (float) MyFunc.twoBytesToInt(buffer, 24) / 1000f;
                    if (buffer[23] == 1) {
                        value4 = -value4;
                    }
                    float value5 = (float) MyFunc.twoBytesToInt(buffer, 27) / 1000f;
                    if (buffer[26] == 1) {
                        value5 = -value5;
                    }
                    value = (value1 + value2 + value3 + value4 + value5) / 5f;

//                    Log.i("ddd", "value（传感器的值）====== " + value);
                    //在校零时，record_zdzdl就是静止时的偏差,用读到的值减去偏差才是实际的值
                    currentRes = value - record_zdzdl;
                    tvLi.setText(df2.format(currentRes));
                    testResLists.add(currentRes);//拉力值集合
                    maxFaceShow = currentRes > maxFaceShow ? currentRes : maxFaceShow;
